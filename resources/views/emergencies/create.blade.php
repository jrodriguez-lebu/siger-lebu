...
@section('page-title', 'Nueva Emergencia')
@section('page-subtitle', 'Registrar una nueva emergencia en el sistema')

@section('content')
<div class="max-w-3xl mx-auto">
    <div class="card">
        <div class="card-header">
            <h2 class="font-semibold text-gray-900">Formulario de Ingreso</h2>
            <a href="{{ route('emergencies.index') }}" class="text-sm text-gray-500 hover:text-gray-700">← Volver</a>
        </div>
        <div class="card-body">
            <form method="POST" action="{{ route('emergencies.store') }}" class="space-y-5">
                @csrf
                ...
                <div>
                    <label class="form-label">Título / Descripción breve <span class="text-red-500">*</span></label>
                    <input type="text" name="title" value="{{ old('title') }}" class="form-input @error('title') border-red-500 @enderror"
                           placeholder="Ej: Incendio en vivienda calle Los Robles">
                    @error('title') <p class="form-error">{{ $message }}</p> @enderror
                </div>

                <div>
                    <label class="form-label">Descripción detallada <span class="text-red-500">*</span></label>
                    <textarea name="description" rows="4" class="form-input @error('description') border-red-500 @enderror"
                              placeholder="Detalla la situación, personas involucradas, estado actual...">{{ old('description') }}</textarea>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="text" name="address" id="address" value="{{ old('address') }}" class="form-input @error('address') border-red-500 @enderror"
                               placeholder="Calle Arturo Prat 123">
                        @error('address') <p class="form-error">{{ $message }}</p> @enderror
                    </div>
                    <div>
                        <label class="form-label">Sector</label>
                        <input type="text" name="sector" value="{{ old('sector') }}" class="form-input" placeholder="Centro, Millongue...">
                    </div>
                </div>
                ...
                <div>
                    ...
                    <p class="text-xs text-gray-400 mb-2">Haz clic en el mapa para marcar el lugar exacto de la emergencia</p>
                    <div id="location-map" class="w-full rounded-lg border border-gray-200 overflow-hidden" style="height: 320px;"></div>
                    <div id="coords-display" class="mt-2 hidden text-xs text-gray-500 flex items-center gap-2">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                    <div>
                        ...
                        <input type="text" name="reported_by_name" value="{{ old('reported_by_name') }}" class="form-input" placeholder="Nombre del reportante">
                    </div>
                    <div>
                        <label class="form-label">Teléfono</label>
                        <input type="text" name="reported_by_phone" value="{{ old('reported_by_phone') }}" class="form-input" placeholder="+56 9...">
                    </div>
                    <div>
                        <label class="form-label">Personas afectadas <span class="text-red-500">*</span></label>
                        <input type="number" name="affected_people" value="{{ old('affected_people', 1) }}" min="0" class="form-input">
                ...
                <div>
                    ...
                    <textarea name="notes" rows="2" class="form-input" placeholder="Notas para el coordinador...">{{ old('notes') }}</textarea>
                </div>

                <div class="flex items-center justify-end gap-3 pt-4 border-t border-gray-100">
                    <a href="{{ route('emergencies.index') }}" class="btn-secondary">Cancelar</a>
                    <button type="submit" class="btn-danger">🚨 Registrar Emergencia</button>
                </div>
            </form>
        </div>
    </div>
...
<script>
(function () {
    // Coordenadas centro de Lebu
    const LEBU = [-37.6089, -73.6524];
    const map = L.map('location-map').setView(LEBU, 14);
    ...
    // Si hay coordenadas previas (old()), mostrar marcador
