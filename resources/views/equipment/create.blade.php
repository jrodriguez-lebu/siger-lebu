@extends('layouts.app')
@section('title', 'Nuevo Equipamiento')

@section('content')
<div class="max-w-2xl mx-auto">
    <div class="card">
        <div class="card-header">
            <h2 class="font-semibold text-gray-900">Nuevo Equipamiento</h2>
            <a href="{{ route('equipment.index') }}" class="text-sm text-gray-500 hover:text-gray-700">← Volver</a>
        </div>
        <div class="card-body">
            <form method="POST" action="{{ route('equipment.store') }}" class="space-y-5">
                @csrf
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div class="sm:col-span-2">
                        ...
                        <input type="text" name="name" value="{{ old('name') }}"
                               ...
                               placeholder="Ej: Desfibrilador AED">
                    ...
                    <div>
                        ...
                        <input type="text" name="serial_number" value="{{ old('serial_number') }}" class="form-input font-mono">
                    </div>
                    ...
                    <div>
                        ...
                        <select name="status" class="form-select @error('status') border-red-500 @enderror">
                            ...
                            <option value="en_uso"       {{ old('status') === 'en_uso'       ? 'selected' : '' }}>En Uso</option>
                    ...
                    <div>
                        ...
                        <input type="date" name="last_maintenance" value="{{ old('last_maintenance') }}" class="form-input">
                    </div>

                    <div>
                        <label class="form-label">Próximo mantenimiento</label>
                        <input type="date" name="next_maintenance" value="{{ old('next_maintenance') }}" class="form-input">
                    </div>

                    <div class="sm:col-span-2">
                        <label class="form-label">Descripción</label>
                        <textarea name="description" rows="2" class="form-input" placeholder="Descripción del equipo...">{{ old('description') }}</textarea>
                    </div>
                ...
                <div class="flex items-center justify-end gap-3 pt-4 border-t border-gray-100">
                    ...
                    <button type="submit" class="btn-primary">Crear Equipamiento</button>
                </div>
            </form>
        </div>
    </div>
